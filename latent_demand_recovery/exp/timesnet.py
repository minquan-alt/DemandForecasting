...
class Config:
    def __init__(self):
        ...
        self.c_out = 1   # output: chỉ predict hours_sale (feature đầu tiên)
        self.d_model = 64
...
# prepare mask
# cần convert thành: 0=missing, 1=observed
mask = np.ones((train_set.shape[0], train_set.shape[1], train_set.shape[2]), dtype=np.float32)
mask[:, :, 0] = (~valid_idx[:, :, 0]).astype(np.float32)  # Feature đầu tiên: invert True→0, False→1

# fill missing values trong train_set
train_set_filled = train_set.copy()
train_set_filled[valid_idx[:, :, 0], 0] = 0  # fill missing hours_sale = 0
...
for epoch in range(num_epochs):
    ...
    for i in range(0, len(train_indices), batch_size):
        ...
        target = torch.FloatTensor(train_set[batch_idx, :, 0]).to(device)  # Ground truth
    ...
    with torch.no_grad():
        for i in range(0, len(val_indices), batch_size):
            ...
            x_dec = x_enc.clone()
            batch_mask = torch.FloatTensor(mask[batch_idx]).to(device)
            target = torch.FloatTensor(train_set[batch_idx, :, 0]).to(device)
